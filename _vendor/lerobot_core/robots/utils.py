...
"""Robot helpers used by the vendored SO follower.

Only :func:`ensure_safe_goal_position` is needed by the SDK; the upstream
``make_robot_from_config`` factory (and its ``import_utils`` dependency) is not
vendored because the SDK instantiates :class:`SOFollower` directly.
"""
